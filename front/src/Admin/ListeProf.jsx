...
export default function ListeProf() {
    ...
    ProfDetails && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white p-6 rounded-lg shadow-md w-96">
          <h2 className="text-lg font-semibold mb-4">Information Professeur</h2>
          <div className="mb-4">
          ...
          <div className="mb-4">
            <label htmlFor="file" className="block text-sm font-medium  text-red-600">
              Etat d'envoi:
            </label>
            <input
              type="text"
              id="email"
              className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
              value={Verification(ProfDetails['email']) ? 'Envoyer' : 'Non envoyé'}
            />
          </div>
          <div className="mb-4">
            <label htmlFor="file" className="block text-sm font-medium  text-red-600">
          ...
          <div className="flex justify-center">
            <button onClick={CLoseWindow} className="ml-2 text-white  px-4 py-2 rounded-md bg-blue-500">
              Annuler
            </button>
          </div>
        </div>
  ...
  useEffect(() => {
    axios
      .get('http://localhost:3001/admin/information-prf')
      .then((response) => {
    ...
    axios
      ...
      .then((res) => {
        if (res.data.message === 'success') {
          ...
          setEnvoyer(true);
          setEnvoyeProf(selectedProf.map((prof) => prof['email'])); // Mettre à jour les professeurs envoyés
  ...
  return (
    <div className="page-professeur relative bg-gray-500 bg-opacity-5">
      ...
      <h1 className="font-bold text-3xl mb-4 text-black mt-4 text-center">Liste des Professeurs</h1>
      ...
      <div className="search-bar">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            id="table-search"
            className=" bg-white text-black text-sm w-64 h-9 rounded-sm border-none search placeholder:text-black"
      ...
      <div className="table-container ">
        <div
          className="relative overflow-x-auto shadow-md sm:rounded-lg table-prof mt-4 "
            ...
            <tbody>
                  ...
                  >
                    ...
                    {Object.values(row).map((value, index) => (
                      <td
                        ...
                        onClick={(e) => OpenWindow(e, row)}
                      >
                        {value}
                      </td>
                    ))}
                    <td className="text-black bg-white ">
                      {envoyer && envoyeProf.includes(row['email']) || Verification(row['email']) ? (
                        <div>
                          <p>Envoyé</p>
              ...
              <tr>
                <td colSpan="6" className="px-6 py-4 text-center">
                  {prof.length === 0 ? <h4 className="font-bold text-black">Pas de fichier sélectionné ?</h4> : ''}
                </td>
              </tr>
            </tbody>
          </table>
