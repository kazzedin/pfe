...
const DocsPage = () => {
  ...
  const handelDeleteSubmit = async (e,filename) => {
    e.preventDefault();
    try {
  ...
  return (
    <div className="container mx-auto mt-8">
      <div className="flex items-center">
        <Link to="/Admin" className="text-black flex items-center hover:text-red-500 mt-2">
      ...
      <button onClick={() => setShowAddModal(true)} className="bg-blue-500 text-white px-4 py-2 rounded-md mb-4 inline-block ml-4">Ajouter un document +</button>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2">
        {docs.map((document) => (
          <div key={document._id} className="max-w-sm rounded overflow-hidden shadow-lg">
            ...
            <div className="px-6 py-4">
              <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full mr-2" onClick={(e) => handelDeleteSubmit(e,document.file.filename)}>Supprimer</button>
              <button onClick={() => handleEditModalOpen(document)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">Modifier</button>
            </div>
          </div>
        ))}
      </div>
