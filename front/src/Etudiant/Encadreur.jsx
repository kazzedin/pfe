...
export default function Encadreur() {
  ...
  const [encadreurDetails, setEncadreurDetails] = useState(null);
  ...
  const handleOpenWindow = (e, info) => {
    e.preventDefault();
    setEncadreurDetails(info);
    axios.get(`http://localhost:3001/etudiant/get-theme-ref-enc/${encadreurDetails['theme']}`)
      .then(res => {
        setShowModal(true);
  ...
  // Fonction pour envoyer une invitation à l'étudiant directement depuis le tableau (personne que vous ne connaissez pas)
  const handleEnvoi = (e, enc_info) => {
    e.preventDefault();
    axios.post('http://localhost:3001/etudiant/envoi-demande-enc', { recever: enc_info, sender_info: info, sender: EtudiantUserEmail })
      .then(res => {
        if (res.data.message === 'success') {
          alert('Votre invitation de binôme a été bien envoyée');
        }
      })
      .catch(err => console.log(err));
  };

  const Window = showModal && encadreurDetails && (
    <div className="fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-10 flex justify-center items-center ">
      <div className="bg-white p-6 rounded-lg shadow-md w-96 fenetre">
        <div className=' flex flex-row justify-between items-center  '>
          <h2 className="text-lg font-semibold mb-4">Informations Étudiant</h2>
          <div className='relative'>
            ...
            {showReferenceTooltip && (
              <div className="absolute bg-gray-700 text-white px-2 py-1 rounded-md text-xs left-0 w-72 flex flex-col items-start justify-start">
                Vous pouvez copier la référence de thème et rechercher ce thème dans la liste des PFE pour avoir plus d'informations sur le thème
              </div>
            )}
          </div>
        </div>
        <div className="mb-4">
          ...
          <input type="text" id="title" readOnly className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value={encadreurDetails['nomPrenom']} />
        </div>
        <div className="mb-4">
          <label htmlFor="category" className="block text-sm font-medium text-gray-700">Section :</label>
          <input type="text" name="section" id="section" value={encadreurDetails['section']} readOnly />
        </div>
        <div className="mb-4">
          <label htmlFor="file" className="block text-sm font-medium text-gray-700">Filière :</label>
          <input type="text" id="filier" className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value={encadreurDetails['filier']} />
        </div>
        <div className="mb-4">
          <label htmlFor="file" className="block text-sm font-medium text-gray-700">Email :</label>
          <input type="text" id="email" className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value={encadreurDetails['email']} />
        </div>
        <div>
          <div className='flex flex-col gap-3'>
            <div>
              <label htmlFor="titreTheme">Titre du thème :</label>
              <input type="text" id='theme' className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value={encadreurDetails['theme'] === null ? `pas d'information?`  : encadreurDetails['theme']} />
            </div>
            <div className='mb-2'>
      ...
      return (
        <div className='binômes relative'>
          ...
          <h1 className='font-bold text-2xl mb-4 text-black mt-4 text-center' style={{ textDecoration: 'underline' }}>Encadreurs</h1>
                  ...
                  {encadreur ? encadreur
                    ...
                    .map((row, index) => (
                      <tr key={index} className='bg-white border-b transition duration-300 hover:bg-gray-50'>
                        ...
                        <td className='px-6 py-4 text-black cursor-pointer text-center' onClick={(e) => handleOpenWindow(e, row)}>
                          {row['theme'] === null ? 'Pas de thème' : row['theme']}
                        </td>
                      </tr>
                    )) : <tr><td colSpan="6">Pas d'encadreur</td></tr>}
                </tbody>
              </table>
              {Window}
            </div>
